...
펜·연필·마커·붓·형광펜·스프레이·지우개 7종과 스포이드·페인트통·직선·사각형·원·화살표를 제공한다. 메인 `단어 만들기`는 영어·한글·일본어·중국어 단어를 무작위로 골라 캔버스에 큰 빈 윤곽선으로 표시해 직접 따라 그릴 수 있게 한다. 브러시별 색상·굵기·농도·흐름·경도·간격·손떨림 보정과 스타일러스 압력을 지원한다. `새 그림`은 별도 프로젝트로 1080×1080 정사각형 작업을 만들며 이전 작업은 `손끝` 버튼의 작업 목록에서 다시 열 수 있다.
